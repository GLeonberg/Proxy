...
static const char* progName = "thread";
static FILE* txtlog;
static int fd;

// struct for passing info to thread
struct info
{
	int cfd;
...
int main(int argc, char* argv[])
{
	...
	while(1)
	{
		// create struct for thread on heap
		struct info* inf = malloc(sizeof(struct info));

		// accept request, store client info
		int client_size = sizeof(struct sockaddr_in);
		if( (inf->cfd = accept(fd, (struct sockaddr*) &(inf->client_addr), &client_size)) < 0)
			err();

		// make thread to handle request
		printf("Making thread to handle request\n");
		pthread_t thread;
		pthread_create(&thread, NULL, handleClient, (void*) inf);
...
void* handleClient(void* param)
{
		int cfd = ((struct info*) param) -> cfd;
		struct sockaddr_in* client_addr = &(((struct info*) param) -> client_addr);
